...
public class AdminController {
    ...
    public List<ComandaDTO> getAllOrders() {
        return comandaService.getAllOrders();
    }

    @DeleteMapping("/deleteOrder/{id}")
    public String deleteOrder(@PathVariable Long id) {
        return comandaService.deleteOrder(id);
    }
    ...
    public ResponseEntity<UserDTO> login(@RequestBody User user) {
        if( userService.loginAdmin(user.getUsername(), user.getPassword()) == null)
            ...
            return ResponseEntity.ok(userService.loginAdmin(user.getUsername(), user.getPassword()));
    }

    @GetMapping("/reviews/{id}")
    public List<ReviewDTO> getReviewsByPerfumeId(@PathVariable Long id) {
        return reviewService.getAllReviewsByPerfumeId(id);
    }
